...
//		}
		...
		/*
		 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
		 * Logic to Udpdate Yearly Details and Quarterly Details
		 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++
		 */
		...
		for (int loop = 0; loop < numOfRecord; loop++) {
			...
			// The dump File path
			System.out.println("Download File Name : "+Fileutils.GetCompanyNameAt(loop, 0));
			System.out.println("Company Name from EQUITY Sheet : "+Fileutils.GetCompanyNameAt(loop, 1));
			String DumpFilePath = Filemodels.getStockDumpsSourcePath() + Fileutils.GetCompanyNameAt(loop, 0).trim()
					+ ".xlsx";
			System.out.println("Searching company (Taken from Equity Sheet) : " + Fileutils.GetCompanyNameAt(loop, 1));
			System.out.println("Searching company ID (Taken from Equity Sheet) : " + Fileutils.GetCellDataFromSheet(loop+1, 0, Filemodels.getMainEquitySheet(),
					Filemodels.getDefaultSheetName()));
			...
			System.out.println("First letter of the company : "+CompanyNameLetter.toUpperCase());
			
			try {
				
				System.out.println("The row count in the File mapped to the seperate sheet : "+Fileutils.GetRowCount(Filemodels.getMainEquitySheet(),
...
//							CompanyNameLetter.toUpperCase());
//					System.out.println("Company Name from EQUITY_LIST Sheet : " + NameFromMainNameSheet.toLowerCase());
//					System.out.println("Company Name from Source Company Names File name : "+Fileutils.GetCompanyNameAt(loop, 0).toLowerCase());
...
//						System.out.println(" Company from Anu sheet : " + Fileutils.GetCompanyNameAt(loop, 0));
...
//				if (Flag) {
					...
					System.out.println(
							"Current working company from Equity sheet : " + Fileutils.GetCompanyNameAt(loop, 1));

					// Sending the file path and company name to the parsing method
					Fileutils.GetDumpData(DumpFilePath, Fileutils.GetCompanyNameAt(loop, 0), Fileutils.GetCompanyNameAt(loop, 1).trim(), Fileutils.GetCellDataFromSheet(loop+1, 0, Filemodels.getMainEquitySheet(),
							Filemodels.getDefaultSheetName()));
//				} else {
...
//						NO_OF_SHARES = Fileutils.GetMoneyValue(Fileutils.GetCellDataFromSheet(5, 1, DumpFilePath,
...
//		      
//				// To Update in Heroku change the credentials in DBoperations class
//				
